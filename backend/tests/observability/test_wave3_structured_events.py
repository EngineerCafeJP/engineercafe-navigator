...
def test_stt_qwen_complete_transcript_is_env_gated(monkeypatch):
    monkeypatch.delenv("STT_LOG_TRANSCRIPT", raising=False)
    hidden = structured_logger_module.build_stt_qwen_complete_payload(
        provider="qwen-primary",
        language="ja",
    ...
    visible = structured_logger_module.build_stt_qwen_complete_payload(
        provider="qwen-primary",
        language="ja",
        audio_duration_ms=1000,
        latency_ms=1200,
    ...
    assert visible["transcript"] == "2階の会議室"


def test_transcript_fields_mask_pii(monkeypatch):
    monkeypatch.setenv("STT_LOG_TRANSCRIPT", "true")
    stt = structured_logger_module.build_stt_qwen_complete_payload(
        provider="qwen-primary",
        language="ja",
        audio_duration_ms=1000,
        latency_ms=1200,
        ...
        transcript="メールは visitor@example.com です",
        winner=True,
    )
    chat = structured_logger_module.build_chat_response_payload(
        request_id="req-chat-pii",
        language="ja",
        metadata={"query": "visitor@example.com に連絡して"},
        latency_ms=1,
    )

    assert stt["transcript"] == "メールは [REDACTED_EMAIL] です"
...
def test_chat_response_adds_wave3_routing_fields():
    ...
    assert payload["transcript"] == "2階の会議室について"
