...
### Phase 0: RouterAgent削除 + エージェント統合準備 `cc:DONE`

> 完了日: 2026-02-15
> PR: #78

- [x] RouterAgent削除（264行、OrchestratorAgentに完全統合済み）
- [x] test_router_agent.py 削除（225行）
- [x] test_router_agent_integration.py → test_orchestrator_integration.py リネーム
- [x] __init__.py, conftest.py からRouterAgent参照削除
- [x] routing_constants.py 未使用定数4件削除（55行）
- [x] clarification_agent_integration.py デッドテスト条件修正（Codex review）
- [x] README.md エージェント数 10→9 更新
- **合計削除**: -793行
...
| タスク | PR |
|--------|-----|
| RouterAgent完全削除（-793行） | #78 |
| routing_constants.py 未使用定数4件削除 + デッドテスト条件修正 | #78 |
| X投稿分析に基づくルーティングキーワード拡張 | #76 |
| ClarificationAgent実践メッセージ化 | #76 |
| ゴールデンデータセット拡張（38→60ケース、100%正解） | #76 |
| RAGカテゴリボーナスバグ修正 + 類似度閾値チューニング | #76 |
| ナレッジベース拡充 20→60エントリ | #76 |
| 施設プロンプト改善 | #76 |
| E2E回答品質評価ツール作成 + 1/6→10/10 改善 | #76 |
| スタッフ検証済み知識データ更新 | #76 |
...
| 項目 | 状態 |
|------|------|
| MAKER's素材・材料費の具体価格 | ✅ フィラメント2円/g、持込禁止、学生無料 |
| バリアフリー情報 | ✅ テラス側スロープ、1F限定、多目的トイレなし |
| 子連れ利用・騒音ポリシー | ✅ 年齢制限なし、子供用椅子/授乳室なし |
| 撮影ポリシー | ✅ スナップOK、三脚/フラッシュ要許可 |
| 初回利用登録の詳細フロー | ✅ 5-10分、Web入力、ID不要 |
| 福岡空港/博多駅からのアクセス | ✅ 空港11分260円、博多6分210円 |
| レーザーカッター利用可能素材 | ✅ アクリル/木材OK、PVC禁止 |
| 近隣宿泊施設案内 | ✅ 西鉄グランド、モントレ等 |
| 英語施設概要 | ✅ スライドナレーションJSON準拠 |
| 季節ごとの施設特徴 | ✅ 夏暑い/冬寒い、地下推奨 |
| Connpass・イベント参加方法 | ✅ 3公式URL |
| EFC（エンジニアフレンドリーシティ福岡） | ✅ efc.fukuoka.jp |
| 飲料・ウォーターサーバー | ✅ なし、サイノカフェで購入 |
